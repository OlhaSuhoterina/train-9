...
function findElementGreaterThan(arr, num) {
  ...
  let foundElement = 0;
  if ((foundElement = arr.find((element) => element > num))) {
    return foundElement;
  } else {
...
function findIndexByProperty(arr, name) {
  ...
  let findName = name;
  const foundIndex = arr.findIndex(({ name }) => name === findName);
  return foundIndex;
...
function findLastEvenNumber(arr) {
  ...
  if ((lastEvenNumber = arr.findLast((element) => element % 2 === 0))) {
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  let arrSlice = arr.slice(startIdx);
  let subArrayString = arrSlice.join(" ");
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = arrayValues.filter((element) => element > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const arrSplice = arr.splice(startIdx);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let arrMap = arrFlat.map((element) => element ** 2);
  let filteredSquaredArray = arrMap.filter((element) => element > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let arrSlice = arr.slice(n);

  const initValue = 0;
  const sum = arrSlice.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    initValue
  );
  arrSlice.forEach((element) => console.log(element));
  return sum;
...
console.log(getLastElementsSumAndPrint([1, 2, 3, 4, 5], 3)); // Виведе
// 4
// 5
// 9
...
function expandArrayByN(arr, n) {
  ...
  let expandedArray = arr.flatMap((element) =>
    element === element ? [element, element, element] : n
  );
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce(function (longest, currentWord) {
...
function calculateTotalPrice(arr) {
  ...
  let totalPrice = arr.reduce(function (sum, { price }) {
    return sum + price;
  }, 0);
